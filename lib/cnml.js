        ...
        node.device && node.device.forEach(function(device) {
          device.radio && device.radio.forEach(function(radio) {
            ...
            radio.interface && radio.interface.forEach(function(interface) {
              interfaces.push([Number(interface.$.id), interface.$.mac, ipv4ShortForm(interface)]);
              if (interface.link) interface.link.forEach(function(link) {
            ...
            radios.push([getRadioModeID(radio.$.mode), radio.$.ssid, Number(radio.$.channel)||null, interfaces]);
          });
        });
